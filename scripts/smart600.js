...
export async function main(ns) {
    ...
    const targets = [
        "crush-fitness", "johnson-ortho", "the-hub", "computek", 
        "netlink", "rothman-uni", "summit-uni", "catalyst", "millenium-fitness", 
        "aevum-police", "rho-construction", "alpha-ent", "syscore"
    ];
    ...
    while (true) {
        ...
        if (bestTarget) {
            ...
            if (weakenNeed > 5) {
                ...
                await ns.exec("weaken.js", "PBweak1", 1, name);
            } else if (growRatio > 1.2) {
                ns.print(`🌱 Growing ${name}`);
                await ns.exec("grow.js", "PBgrow1", 1, name);
            } else {
                ns.print(`💸 Hacking ${name}`);
                await ns.exec("hack.js", "PBhack1", 1, name);
            }
        }
